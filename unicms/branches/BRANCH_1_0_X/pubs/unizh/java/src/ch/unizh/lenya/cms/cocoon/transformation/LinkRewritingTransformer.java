...
public class LinkRewritingTransformer extends AbstractSAXTransformer implements Disposable {
            ...
            if (href != null) {
                ...
                try {
                    ...
                    if (href.startsWith(context + "/" + publication.getId())) {
                        ...
                        String webappUrlWithRequestParams = null;
                        String webappUrl = null;
                        
                        int anchorIndex = webappUrlWithAnchor.indexOf("#");
                        if (anchorIndex > -1) {
                            webappUrlWithRequestParams = webappUrlWithAnchor.substring(0, anchorIndex);
                            anchor = webappUrlWithAnchor.substring(anchorIndex + 1);
                        }
                        else {
                            webappUrlWithRequestParams = webappUrlWithAnchor;
                        }
                        
                        if (getLogger().isDebugEnabled()) {
                            getLogger().debug(this.indent + "webapp URL with Params: [" + webappUrlWithRequestParams + "]");
                            getLogger().debug(this.indent + "anchor:     [" + anchor + "]");
                        }

                        int requestParamsIndex = webappUrlWithRequestParams.indexOf("?");
                        if (requestParamsIndex > -1) {
                            webappUrl = webappUrlWithRequestParams.substring(0, requestParamsIndex);
                            requestParams = webappUrlWithRequestParams.substring(requestParamsIndex + 1);
                        }                    
                        else {
                            webappUrl = webappUrlWithRequestParams;
                        }
                        ...
                        if (builder.isDocument(publication, webappUrl)) {
                            ...
                            if (targetDocument.exists()) {
                                rewriteLink(newAttrs, targetDocument, anchor, requestParams);
                            } else {
     ...
     */
    protected void rewriteLink(AttributesImpl newAttrs, Document targetDocument, String anchor, String requestParams)
