...
<xsl:template match="unizh:newsitem">
<node name="Title" select="/unizh:newsitem/lenya:meta/dc:title[@tagID='{lenya:meta/dc:title/@tagID}']">
  <content><input type="text" name="&lt;xupdate:update select=&quot;/unizh:newsitem/lenya:meta/dc:title[@tagID='{lenya:meta/dc:title/@tagID}']&quot;&gt;" size="40"><xsl:attribute name="value"><xsl:value-of select="lenya:meta/dc:title"/></xsl:attribute></input></content>
</node>

<xsl:apply-templates select="unizh:short"/>
...
<xsl:template match="unizh:short">
  ...
  <node name="Short" select="/*/unizh:short[@tagID='{@tagID}']">
    <content>
      <textarea name="&lt;xupdate:update select=&quot;/*/unizh:short[@tagID='{@tagID}']&quot;&gt;" cols="60" rows="30">
	<xsl:copy-of select="node()"/>
      </textarea>
    </content>
  </node>
...
<xsl:template match="xhtml:object">
  <node name="Object">
    <action><delete name="&lt;xupdate:remove select=&quot;//unizh:short/xhtml:object[@tagID='{@tagID}']&quot;/&gt;"/></action>
      <content>
