...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="elml:furtherReading">
    <ul>
      <xsl:apply-templates select="elml:resItem"/>
    </ul>
  </xsl:template>
  ...
  <xsl:template match="elml:box">
    ...
    <table border="0" width="50%" bgcolor="#dedede">
      <tr>
        <xsl:if test="@icon">
          <td>
            <img src="{$imageprefix}/{@icon}.png"/>
    ...
    <p>
      <xsl:choose>
        <xsl:when test="@icon">
          <table border="0">
            <tr>
              <td><img src="{$imageprefix}/{@icon}.png"/></td>
              <td><xsl:apply-templates/></td>
            </tr>
          </table>
        </xsl:when>
  ...
  <xsl:template match="elml:multimedia">
    <xsl:if test="not(@visible='print')">
      <xsl:choose>
        <xsl:when test="((boolean(name(preceding-sibling::*[1])) or boolean(name(following-sibling::*[1]))) and not(../text())) or (count(../*)=number('1') and (name(parent::*)='elml:look' or name(parent::*)='elml:act' or  name(parent::*)='elml:clarify')) and not(name(parent::*)='elml:link')">
          <table cellpadding="0" cellspacing="5" border="0" width="100%" class="content_table">
            <tr>
              <xsl:if test="@icon">
