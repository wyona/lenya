  ...
  <xsl:template match="xhtml:div[@id = 'toolnav']">
    <div id="toolnav">
      ...
      <xsl:variable name="printurl">
        <xsl:value-of select="xhtml:div[@id = 'print']/@href"/>
        <xsl:if test="$current_section = 'unit'">
          <xsl:value-of select="concat('?lesson.section=unit&#38;section.label=', $current_label)"/>
        </xsl:if>
        <xsl:if test="$current_section = 'selfAssessment'">
          <xsl:value-of select="'?lesson.section=selfAssessment'"/>
        </xsl:if>
        <xsl:if test="$current_section = 'summary'">
          <xsl:value-of select="'?lesson.section=summary'"/>
        </xsl:if>
        <xsl:if test="$current_section = 'glossary'">
          <xsl:value-of select="'?lesson.section=glossary'"/>
        </xsl:if>
        <xsl:if test="$current_section = 'bibliography'">
          <xsl:value-of select="'?lesson.section=bibliography'"/>
        </xsl:if>
      </xsl:variable>

      <a href="#" onClick="window.open('{$printurl}', '', 'width=700,height=700,menubar=yes,scrollbars')" onmouseout="changeIcontext('')" onmouseover="changeIcontext('{xhtml:div[@id = 'print']}')"><img src="{$imageprefix}/icon_print.gif" alt="{xhtml:div[@id = 'print']}" width="10" height="10" /></a> |
