...
<xsl:stylesheet version="1.0"
  ...
  <xsl:include href="../common/header-home.xsl"/>
    ...
    <html>
      ...
      <body>
        <div class="bodywidth">
          ...
          <xsl:if test="$document-element-name != 'unizh:homepage4cols' and $document-element-name != 'unizh:homepage'">
          ...
          <xsl:choose>
            <xsl:when test="$document-element-name = 'unizh:homepage4cols' or $document-element-name = 'unizh:homepage'">
              <xsl:call-template name="header-home"/>
            </xsl:when>
            <xsl:otherwise>
  ...
  <xsl:template name="overview">
    <xsl:choose>
      <xsl:when test="count(/document/unizh:ancestors/unizh:ancestor) &gt; 1">
        <div id="col1">
          <xsl:apply-templates select="/document/xhtml:div[@id = 'menu']"/>
        </div>
      </xsl:when>
    ...
    <div>
      <xsl:attribute name="class">
        <xsl:choose>
          <xsl:when test="$numColumns = 3">contcol2, wide</xsl:when>
      ...
      <div>
        <xsl:attribute name="class">
          <xsl:choose>
            <xsl:when test="$numColumns = 3">contentareawide</xsl:when>
  ...
  <xsl:template match="unizh:column[3]">
    <div class="column" id="c3" bxe_xpath="/{$document-element-name}/xhtml:body/unizh:column[3]">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
