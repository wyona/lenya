...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="docId"><xsl:value-of select="/usecase:schedule/usecase:document-id"/></xsl:variable>
  ...
  <xsl:template match="/usecase:schedule">
    ...
    <page:page>
      ...
      <page:body>
        ...
        <sch:scheduler-form>
          <xsl:choose>
            <xsl:when test="$docId='/newsletter'">
              <sch:job tasklabel="Dispatch Newsletter">
                <input type="hidden" name="wrapper.task-id" value="send-newsletter"/>
                <input type="hidden" name="uri" value="/{/usecase:schedule/usecase:area}{/usecase:schedule/usecase:document-id}_{/usecase:schedule/usecase:document-language}.mail.html"/>
          ...
          <sch:job tasklabel="Publish&#160;Subtree">
            <input type="hidden" name="wrapper.task-id" value="publishtree"/>
            <input type="hidden" name="workflow-event" value="publish"/>
            <xsl:call-template name="task-parameters"/>
          </sch:job>
          
          <sch:job tasklabel="Deactivate">
            <input type="hidden" name="wrapper.task-id" value="deactivate"/>
            <input type="hidden" name="workflow-event" value="deactivate"/>
            <xsl:call-template name="task-parameters"/>
          </sch:job>
          
          <sch:job tasklabel="Deactivate&#160;Subtree">
            <input type="hidden" name="wrapper.task-id" value="deactivatetree"/>
            <input type="hidden" name="workflow-event" value="deactivate"/>
            <xsl:call-template name="task-parameters"/>
          </sch:job>
