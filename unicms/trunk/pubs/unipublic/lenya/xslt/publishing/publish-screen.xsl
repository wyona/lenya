...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="usecase:parent">
      <span class="lenya-form-error">This page cannot be published unless its parent is published:</span>
      <ul>
        <li><a href="{@href}"><xsl:value-of select="@id"/> [<xsl:value-of select="@language"/>]</a></li>
      </ul>
  </xsl:template>
        
  <xsl:template match="usecase:checkout">
      <span class="lenya-form-error">This page cannot be published because the document</span>
      <ul>
        <li><xsl:value-of select="."/></li>
      </ul>
      <span>is checked out by another user</span><br/><br/>
  </xsl:template>
                ...
                <xsl:choose>
                  <xsl:when test="usecase:message">
                    ...
                    <td>
                        ...
                        <xsl:apply-templates select="usecase:checkout"/>
