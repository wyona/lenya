...
<xsl:template match="unizh:rss-reader" mode="rc">
  <node name="RSS Feed" select="/*/unizh:related-content/unizh:rss-reader[@tagID='{@tagID}']">
    <action><delete name="&lt;xupdate:remove select=&quot;/*/unizh:related-content/unizh:rss-reader[@tagID='{@tagID}']&quot;/&gt;"/></action>
    <content>
       <input type="text" name="&lt;xupdate:update select=&quot;//unizh:related-content/unizh:rss-reader[@tagID='{@tagID}']/@url&quot;&gt;" size="50"><xsl:attribute name="value"><xsl:value-of select="@url"/></xsl:attribute>
       </input>
    </content>
    <content>
        <textprefix>Number of items:</textprefix>
       <input type="text" name="&lt;xupdate:update select=&quot;//unizh:related-content/unizh:rss-reader[@tagID='{@tagID}']/@items&quot;&gt;" size="5"><xsl:attribute name="value"><xsl:value-of select="@items"/></xsl:attribute>
       </input>
   </content>
    <content>
