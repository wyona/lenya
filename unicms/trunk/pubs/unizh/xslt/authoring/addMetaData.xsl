...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="subhomepageType"/>
  ...
  <xsl:template match="/*[$subhomepageType!='']">
    <xsl:copy>
      <xsl:attribute name="unizh:subhomepageType" namespace="http://unizh.ch/doctypes/elements/1.0"><xsl:value-of select="$subhomepageType"/></xsl:attribute>
      <xsl:apply-templates select="@*[local-name()!='subhomepageType']|node()"/>
    </xsl:copy>
  </xsl:template>  
