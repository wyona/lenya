...
Further you need to download the dms2 pub:
svn co https://svn.apache.org/repos/asf/lenya/sandbox/dms2 dms2

We will refer to the checkouts {lenya-trunk} for lenya and {forrest-trunk} for forrest
and {dms2} for the dms2 pub.
...
you need to 
1) add the pub:
pubs.root.dirs={dms2}:{pubs.root.dirs.more.pubs}
2) add the modules:
modules.root.dirs={dms2}/modules:{modules.root.dirs.more.modules}
...
1) cp {dms2}/config/publication.xconf {dms2}/config/local.publication.xconf
2) change <content-dir src="/home/thorsten/src/apache/forrest-trunk/whiteboard/dms2"/> to
<content-dir src="{forrest-trunk}/whiteboard/dms2"/> for forrest (we will start with this)
...
http://localhost:9999/dms2/authoring/index.html
## Render mode
...
$FORREST_HOME/tools/ant/bin/ant local-deploy
2) Run forrest, run ;)
cd $FORREST_HOME/whiteboard/dms2
forrest run
