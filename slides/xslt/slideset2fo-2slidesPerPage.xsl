  ...
  <xsl:template match="/">
   <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:for-each select="/s:slideset/s:slide">
	  <!-- XSL-FO Blocks: http://www.w3schools.com/xslfo/xslfo_blocks.asp -->
          <fo:block border-width="0.1mm" border-style="solid" font-size="18pt" padding="20pt" space-before="10pt"><xsl:if test="(position() mod 2) = 1"><xsl:attribute name="break-before">page</xsl:attribute></xsl:if>
...
-->
  ...
  <xsl:template match="ol">
    ...
    <xsl:for-each select="li">
        <fo:list-item space-before.optimum="20pt">
