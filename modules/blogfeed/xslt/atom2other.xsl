...
<xsl:stylesheet xmlns:xi="http://www.w3.org/2001/XInclude"
  ...
  xmlns:echo="http://purl.org/atom/ns#" xmlns:rss="http://purl.org/rss/"
  xmlns:ent="http://www.purl.org/NET/ENT/1.0/" version="1.0">
  <xsl:param name="type"/>
  <xsl:param name="url"/>
      ...
      <xsl:when test="$type='rss'">
        <rss version="2.0">
          <channel>
            ...
            <lastBuildDate>
              <xsl:value-of select="./echo:updated/text()"/>
            </lastBuildDate>
      ...
      <link>
        <xsl:choose>
          <xsl:when test="echo:link/@href=''">
            <xsl:value-of select="concat($url, echo:link/@href)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="echo:link/@href"/>
          </xsl:otherwise>
